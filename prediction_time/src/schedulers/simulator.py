...
class Simulator(object):
    ...
    def handle_submission_event(self, event):
        ...
        if event.job.actual_run_time > 0 or event.job.actual_run_time == -1:	#sid
	...
	if event.job in self.event_queue.RunningJobs:
	    rem_job = event.job
	else:
	    for temp_job in self.event_queue.RunningJobs:
		if temp_job.id == event.job.id:
		    rem_job = temp_job

        newEvents = self.scheduler.new_events_on_job_termination(rem_job, event.timestamp)
        self.terminated_jobs.append(rem_job)
        self.event_queue.RunningJobs.remove(rem_job)	
	"""
