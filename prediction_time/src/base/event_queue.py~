...
class EventQueue(object):
	...
	print "add_event: ",event.__class__.__name__," JobId: ",event.job.id," timestamp: ",event.timestamp, ""
        assert (event.timestamp, event) not in self._events_heap, "assert: JobID = %r" % event.job.id # TODO: slow assert, disable for production
