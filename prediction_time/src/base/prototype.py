...
class Job(object):
    ...
    def finish_time(self):
        assert self.start_to_run_at_time != -1, self.__repr__
    ...
    def __repr__(self):
        return type(self).__name__ + "<id=%(id)s, user_estimated_run_time=%(user_estimated_run_time)s, actual_run_time=%(actual_run_time)s, required_processors=%(num_required_processors)s, start_to_run_at_time=%(start_to_run_at_time)s, submit_time=%(submit_time)s>, user_id=%(user_id)s" % vars(self)
...
class Machine(object):
	...
	#to manage real_time_update
	if job in self.event_queue.QueuedJobs:
            self.event_queue.QueuedJobs.remove(job)
	else:
	    for temp_job in self.event_queue.QueuedJobs:
		if temp_job.id == job.id:
		    self.event_queue.QueuedJobs.remove(temp_job)
...
class ValidatingMachine(Machine):
    ...
    def _add_job(self, job, current_timestamp):
	#print "inside prototype.py inside class validatingmachine(object) add job: ", self.jobs
	...
	if event.job in self.jobs:
            self.jobs.remove(event.job)
	else:
	    for temp_job in self.jobs:
		if event.job.id == temp_job.id:
		    rem_job = temp_job
	    self.jobs.remove(rem_job)
...
def _job_input_to_job(job_input, total_num_processors):
	...
	actual_wait_time = int(max(job_input.wait_time, 0)),
        #actual_run_time = int(max(min(job_input.requested_time, job_input.run_time), 1)), #TODO: Change the actual runtime #sid
	actual_run_time = job_input.run_time,
        #actual_run_time = float(max(job_input.run_time,1)),
